				...
				if (!(entityiterator instanceof LivingEntity _livEnt ? _livEnt.hasEffect(CraftkaisenModMobEffects.EMBER_CONTROL.get()) : false)) {
					if (!(entityiterator instanceof EmberInsectEntity)) {
						if (!world.getEntitiesOfClass(LivingEntity.class, AABB.ofSize(new Vec3(x, y, z), 4, 4, 4), e -> true).isEmpty()) {
							...
							if (world instanceof ServerLevel _level)
								_level.sendParticles(ParticleTypes.LAVA, x, y, z, 5, 1, 2, 1, 0.8);
							if (world instanceof ServerLevel _level)
								_level.sendParticles(ParticleTypes.SMOKE, x, y, z, 1, 1, 2, 1, 0.8);
							...
							if (entity.getPersistentData().getDouble("fistTimer") == 0) {
								if (!entity.getPersistentData().getBoolean("fistTag")) {
									entity.getPersistentData().putDouble("fistTimer", 20);
								} else {
