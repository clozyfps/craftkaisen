...
import net.mcreator.craftkaisen.network.EnterLeaveMechMessage;
...
public class CraftkaisenModKeyMappings {
	...
	public static final KeyMapping ENTER_LEAVE_MECH = new KeyMapping("key.craftkaisen.enter_leave_mech", GLFW.GLFW_KEY_B, "key.categories.jjk") {
		private boolean isDownOld = false;
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				CraftkaisenMod.PACKET_HANDLER.sendToServer(new EnterLeaveMechMessage(0, 0));
				EnterLeaveMechMessage.pressAction(Minecraft.getInstance().player, 0, 0);
			}
			isDownOld = isDown;
		}
	};
	...
	public static void registerKeyMappings(RegisterKeyMappingsEvent event) {
		...
		event.register(ENTER_LEAVE_MECH);
	}
