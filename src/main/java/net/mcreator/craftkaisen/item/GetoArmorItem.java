...
import net.mcreator.craftkaisen.client.model.Modelgetochestplate;
	...
	public GetoArmorItem(EquipmentSlot slot, Item.Properties properties) {
		super(new ArmorMaterial() {
			...
			public int getDefenseForSlot(EquipmentSlot slot) {
				return new int[]{2, 8, 9, 3}[slot.getIndex()];
			}
		...
		public void initializeClient(Consumer<IClientItemExtensions> consumer) {
			consumer.accept(new IClientItemExtensions() {
				...
				public HumanoidModel getHumanoidArmorModel(LivingEntity living, ItemStack stack, EquipmentSlot slot, HumanoidModel defaultModel) {
					HumanoidModel armorModel = new HumanoidModel(new ModelPart(Collections.emptyList(), Map.of("body", new Modelgetochestplate(Minecraft.getInstance().getEntityModels().bakeLayer(Modelgetochestplate.LAYER_LOCATION)).Body, "left_arm",
							new Modelgetochestplate(Minecraft.getInstance().getEntityModels().bakeLayer(Modelgetochestplate.LAYER_LOCATION)).LeftArm, "right_arm",
							new Modelgetochestplate(Minecraft.getInstance().getEntityModels().bakeLayer(Modelgetochestplate.LAYER_LOCATION)).RightArm, "head", new ModelPart(Collections.emptyList(), Collections.emptyMap()), "hat",
			...
			consumer.accept(new IClientItemExtensions() {
				...
				public HumanoidModel getHumanoidArmorModel(LivingEntity living, ItemStack stack, EquipmentSlot slot, HumanoidModel defaultModel) {
					HumanoidModel armorModel = new HumanoidModel(new ModelPart(Collections.emptyList(),
							Map.of("left_leg", new Modelgetolegs(Minecraft.getInstance().getEntityModels().bakeLayer(Modelgetolegs.LAYER_LOCATION)).RightLeg, "right_leg",
